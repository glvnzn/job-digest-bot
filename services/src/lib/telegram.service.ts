...
export class TelegramService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.logger.log(`🤖 Telegram bot starting with polling: ${enablePolling}`);
    ...
    this.logger.log('📱 Setting up Telegram commands...');
    this.setupCommands();
...
4. Archives processed emails to keep your inbox clean`);
  ...
  async sendJobNotifications(jobs: JobListing[], isHourlyBatch: boolean = true): Promise<void> {
    if (jobs.length === 0) {
      ...
      this.logger.log('No relevant jobs to send');
      return;
    }
    ...
    try {
      ...
      this.logger.log(`Sent ${jobs.length} job notifications to Telegram`);
    } catch (error) {
...
`;
      ...
      this.logger.log(`Sent daily summary with ${jobs.length} jobs to Telegram`);
    } catch (error) {
...
`;
  ...
  async sendStatusMessage(message: string): Promise<void> {
    try {
      ...
      this.logger.error('Failed to send status message:', error);
    }
  }
  ...
  async createProgressMessage(initialMessage: string): Promise<number | null> {
    try {
      ...
      this.logger.error('Failed to create progress message:', error);
      return null;
    }
  }
  ...
  async updateProgressMessage(messageId: number, newMessage: string): Promise<void> {
    try {
      ...
      this.logger.error('Failed to update progress message:', error);
