...
export class GmailService {
  ...
  async getRecentEmails(): Promise<EmailData[]> {
    try {
      ...
      if (!response.data.messages) {
        this.logger.log('No recent emails found');
        return [];
      }
      ...
      for (let i = 0; i < response.data.messages.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (message: { id: string }) => {
          try {
            ...
            this.logger.error(`Failed to fetch email ${message.id}:`, error);
            return null;
          }
        });
  ...
  private async getEmailDetails(messageId: string): Promise<EmailData | null> {
    try {
      ...
      this.logger.error(`Error fetching email details for ${messageId}:`, error);
      return null;
    }
  }
  ...
  async deleteEmail(messageId: string): Promise<void> {
    try {
      ...
      this.logger.log(`Email ${messageId} deleted successfully`);
    } catch (error) {
      this.logger.error(`Failed to delete email ${messageId}:`, error);
      throw error;
    }
  }
  ...
  async markAsRead(messageId: string): Promise<void> {
    try {
      ...
      this.logger.log(`Email ${messageId} marked as read successfully`);
    } catch (error) {
  ...
  async archiveEmail(messageId: string): Promise<void> {
    try {
      ...
      this.logger.log(`Email ${messageId} archived successfully`);
    } catch (error) {
      this.logger.error(`Failed to archive email ${messageId}:`, error);
      throw error;
    }
  }
  ...
  async markAsReadAndArchive(messageId: string): Promise<void> {
    try {
      ...
      this.logger.log(`Email ${messageId} marked as read and archived successfully`);
    } catch (error) {
      this.logger.error(`Failed to mark email ${messageId} as read and archive:`, error);
      throw error;
    }
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      this.logger.log(`Gmail connection successful for: ${response.data.emailAddress}`);
      return true;
    } catch (error) {
